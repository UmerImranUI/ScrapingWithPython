...
class SfmaSpider(scrapy.Spider):
    ...
    def parse_details(self, response):
        ...
        Phone = sh.cleanup(response.xpath(
            '//strong[text()="Tel:"]/../text()').get())
        Fax = sh.cleanup(response.xpath(
            '//strong[text()="Fax:"]/../text()').get())
        Email = sh.cleanup(response.xpath(
            '//strong[text()="Email:"]/../text()').get())
        Website = sh.cleanup(response.xpath(
            '//strong[text()="Website:"]/../text()').get())
        yield {
            'Name': Name,
            'Address': Address,
            'Phone': Phone,
